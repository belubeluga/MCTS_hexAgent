...
def restart_interpreter():
    try:
        ...
        sys.exit(0)  # Exit the current interpreter
    except subprocess.CalledProcessError as e:
        print(f"Failed to restart interpreter: {e}")
        sys.exit(1)
    ...
    with HiddenPrints():
        ...
        while not terminated and not truncated and len(board[board == 0]) > 0:
            if n % 2 == 0:
                ...
                try:
                    # 1) Transpose and flip
                    board_for_agent2 = board.T * -1
                    N = board_for_agent2.shape[0]

                    # 2) Ask agent2
                    start = time.perf_counter()
                    action_t = agent2.action(board_for_agent2)
                    delta = time.perf_counter() - start
                    time2 += delta

                    # 3) Convert index => row_t, col_t
                    row_t, col_t = divmod(action_t, N)  # N is the board size
                    # 4) Undo transpose => (row_env, col_env)
                    row_env, col_env = col_t, row_t
                    action = row_env * N + col_env
                except Exception as e:
                    agent2.errors += 1
                    logging.exception(
                        f"Error running agent {agent2.name()} against {agent1.name()} while the board is {board}"
...
def process_names(*agents) -> dict:
    """This function processes the names of the agents to remove special characters and return a dictionary with the names of the agents.

    If the agent does not have a __str__ method, it will be named "Player 1", "Player 2", etc., depending on the agent.

    Args:
        *agents: Variable number of GomokuAgent instances.

    Returns:
        dict: Dictionary with the names of the agents using the keys "agent1", "agent2", etc.
    """
    pattern = re.compile(
    ...
    for i, agent in enumerate(agents, start=1):
        if not hasattr(agent, "__str__") or type(agent).__str__ is object.__str__:
            name = f"Player {i}"
        else:
            name = re.sub(pattern, "", str(agent))
            if not name:
                name = f"Player {i}"
        names[f"agent{i}"] = name
